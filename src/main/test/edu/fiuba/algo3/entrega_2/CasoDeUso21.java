...
public class CasoDeUso21 {
	...
	@Test
    public void unMutaliscoNoPuedeEvolucionarAGuardiaSinLosRecursosSolicitados() {
        ...
        jugadorZerg.construirEdificio(new Coordenada(5, 5), new Espiral());
        for (int i = 0; i < 8; i++) {
            algoStar.pasarTurno();
        }
        jugadorZerg.generarUnidad(new Coordenada(2, 2), new Derecha(), new Mutalisco());
        for (int i = 0; i < 14; i++) { // tarda 7
            algoStar.pasarTurno();
        }
        Assertions.assertTrue(jugadorZerg.evolucionar(new Coordenada(5, 5);
    }
